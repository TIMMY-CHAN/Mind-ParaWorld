#!/bin/bash
# run_judge_evaluation.sh - Setting B Guided Search (Few-shot) LLM-as-Judge 评估

set -e

echo "========================================================================"
echo "  Setting B: Guided Search (Few-shot) - LLM-as-Judge 评估"
echo "========================================================================"
echo ""
...
# JUDGE_API_BASE="http://10.72.0.89:8000/v1"
MODEL="Qwen3-30B-A3B"
# MODEL="tongyi_deepresearch"

# 数据配置（修改这里以评估不同模型的结果）
INPUT_JSONL="setting_b_results/fewshot_prompt/$MODEL/inference_results.jsonl"
OUTPUT_JSONL="setting_b_results/fewshot_prompt/$MODEL/evaluated_results.jsonl"
...
python experiments/setting_B_guided/evaluate/llm-as-judge.py \
    --input "$INPUT_JSONL" \
    --output "$OUTPUT_JSONL" \
