#!/usr/bin/env python3
"""
llm-as-judge.py - Setting C: Unguided Search (Zero-shot) 重新评估（LLM-as-Judge）

使用LLM判断模型预测是否与ground truth一致。
只有两种结果：Correct 或 Incorrect

注意：Setting C的数据格式中，question需要从messages中提取
支持重新评估已有evaluation字段的结果文件
"""
...
class SettingCJudgeEngine:
    """Setting C 的 LLM-as-Judge 评估引擎（支持重新评估）"""
    ...
    async def evaluate_all(
        ...
        Args:
            input_file: 输入的evaluated_results.jsonl文件（可能已包含旧的evaluation）
            output_file: 输出的重新评估结果文件
            force_reevaluate: 是否强制重新评估所有样本
        """
        # 1. 加载推理结果
        ...
        # 3. 过滤未评估的样本
        if force_reevaluate:
            samples_to_judge = samples
            print(f"⚠️  强制重新评估模式：将重新评估所有 {len(samples)} 条样本")
        else:
        ...
        for coro in asyncio.as_completed(tasks):
            ...
            # 移除旧的evaluation字段（如果存在）
            if 'evaluation' in original_sample:
                del original_sample['evaluation']
...
async def main():
    parser = argparse.ArgumentParser(description="Setting C: Unguided Search - LLM-as-Judge 重新评估")
    parser.add_argument('--input', required=True, help='输入的evaluated_results.jsonl文件')
    parser.add_argument('--output', required=True, help='输出的重新评估结果文件')
    ...
    parser.add_argument('--max-tokens', type=int, default=8192, help='Judge最大token数（默认8192）')
    parser.add_argument('--temperature', type=float, default=0.8, help='Judge温度（默认0.8）')
    parser.add_argument('--max-retries', type=int, default=3, help='失败重试次数（默认3）')
    parser.add_argument('--disable-thinking', action='store_true', help='禁用thinking模式')
    parser.add_argument('--force-reevaluate', action='store_true', help='强制重新评估所有样本')

    args = parser.parse_args()

    # 初始化Judge引擎
    engine = SettingCJudgeEngine(
